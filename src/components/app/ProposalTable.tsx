        ...
        {proposals.map((row) => (
          <TableRow key={row.id}>
            ...
            <TableCell className="min-w-0">
              <div className="font-semibold text-neutral-900 min-w-[120px] max-w-md whitespace-normal break-words line-clamp-none md:line-clamp-2 leading-snug" title={row.title}>
                {row.title}
              </div>
            </TableCell>
            ...
            {renderActions && (
              <TableCell className="text-right space-x-2 whitespace-nowrap md:sticky md:right-0 bg-white md:shadow-[-10px_0_15px_-10px_rgba(0,0,0,0.1)] z-10 group-hover:bg-muted/50 transition-colors">
                {renderActions(row)}
              </TableCell>
            )}
          </TableRow>
